...
const IndexScreen = ({navigation}) => {
  ...
  // Fetch posts only on mount or navigation focus
  useEffect(() => {
    const fetchPosts = async () => {
      console.log('Fetching posts from the server...');
      await getDiaryPosts(); // Fetch posts on component mount
    };

    fetchPosts();

    const listener = navigation.addListener('focus', fetchPosts); // Replace 'didFocus' with 'focus' for current React Navigation
    return () => {
      listener(); // Clean up listener
    };
  }, []); // Empty dependency array ensures this runs only once

  // Update grouped posts when `state` changes
  useEffect(() => {
    setGroupedPosts(groupPostsByLanguage(state));
  }, [state]); // This effect only runs when `state` changes
